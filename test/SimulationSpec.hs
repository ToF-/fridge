...
spec = do
    describe "A simulation" $ do
        it "is initially empty" $ do
            ...
            (situationState "ToF" sim) `shouldBe` Left "no situation exists with name:ToF"

        it "can get the state of a situation once created" $ do
            let sim = addSituation "ToF" newSimulation
            (sim >>= (situationState "ToF")) `shouldBe` Right (Halted, 15.0, 100)
        ...
        it "can start a situation once created" $ do
            let sim = addSituation "ToF" newSimulation
            (sim >>= apply start "ToF" >>= situationState "ToF") `shouldBe` Right (Started, 15.0, 100)
            (sim >>= apply start "Ben") `shouldBe` Left "no situation exists with name:Ben"

        it "can change a situation once created" $ do
            let sim = addSituation "ToF" newSimulation
            (sim >>= apply start "ToF" >>= changeSituation 50 "ToF" >>= situationState "ToF") 
        ...
        it "cannot change a situation that is not started" $ do
            let sim = addSituation "ToF" newSimulation
            (sim >>= changeSituation 50 "ToF" >>= situationState "ToF") 
        ...
        it "can halt a situation once created" $ do
            ...
            (sim >>= apply halt "ToF" >>= situationState "ToF") `shouldBe` Right (Halted, 15.0, 100)
        ...
        it "can reset a situation once created" $ do
            ...
            (sim >>= apply reset "ToF" >>= situationState "ToF") `shouldBe` Right (Halted, 15.0, 100)
            (sim >>= apply reset "Ben") `shouldBe` Left "no situation exists with name:Ben"

        it "can tick a situation once created" $ do
            let sim = (addSituation "ToF" newSimulation) >>= apply start "ToF"
            (sim >>= apply tick "ToF" >>= situationState "ToF") `shouldBe` Right (Started, 14.0, 100)
        ...
        it "can start all situations" $ do
            ...
            (sim >>= situationState "ToF") `shouldBe` Right (Started, 15.0, 100)
            (sim >>= situationState "Ben") `shouldBe` Right (Started, 15.0, 100)

        it "can halt all situations" $ do
            let sim = return newSimulation >>= addSituation "ToF" >>= addSituation "Ben" >>= applyAll start >>= applyAll halt
            (sim >>= situationState "ToF") `shouldBe` Right (Halted, 15.0, 100)
            (sim >>= situationState "Ben") `shouldBe` Right (Halted, 15.0, 100)
        ...
        it "can tick all situations that are started" $ do
            ...
            (sim >>= situationState "ToF") `shouldBe` Right (Started, 14.0, 100)
            (sim >>= situationState "Ben") `shouldBe` Right (Started, 14.0, 100)
            (sim >>= situationState "Gus") `shouldBe` Right (Halted, 15.0, 100)
