...
spec = do
    describe "A room" $ do
        ...
        it "has its temperature evolving depending on cursor position" $ do
            temperature (evolve (change 50 openRoom))
                `shouldBe` 12.333333333333334
            temperature (evolve (evolve (change 200 openRoom)))
        ...
        it "cannot evolve or change in closed state" $ do
            ...
            position (change 17 newRoom) `shouldBe` 100
